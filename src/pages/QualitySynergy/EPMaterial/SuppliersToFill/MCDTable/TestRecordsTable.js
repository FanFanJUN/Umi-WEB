...
const supplierModal = forwardRef(({ form, selectedSplitData, handleSplitDataList, environmentalProtectionCode, isView, isImport }, ref) => {
  ...
  // 新增/编辑弹框
  async function showEditModal (type) {
    setModalType(type)
    setVisible(true)
  }
  ...
  return <Fragment>
    ...
    <div className={classnames({
      ...
      <Button className={styles.btn} onClick={handleDelete} key="delete"
        disabled={(selectedRowKeys.length === 0 || selectedRows.find(item => item.isFlgRequired))}
      >删除</Button>
          ...
          <FormItem label='物质名称' {...formLayout}>
            {
              ...
              })(<ComboList disabled={modalType === 'edit' && selectedRows[0].isFlgRequired} form={form}
