...
const supplierModal = forwardRef(({ form, selectedRow, supplierModalType, viewDemandNum, refreshTable, ids }, ref) => {
  ...
  function handleAdd(tag) {
    ...
    supplierSelected.forEach((item, index) => {
      if (!supplierCodes.includes(item.code) || ids.length !== 1) {
  ...
  function handleSave() {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        console.log(ids, 'selectedRowKeysselectedRowKeys');
        let saveData = { ...selectedRow, demandSupplierBoList: saveList, demandIds: ids.toString() };
    ...
    >
      <div className={styles.mbt} style={{ display: supplierModalType === 'distribute' ? 'flex' : 'none' }}>
        ...
        <Button className={styles.btn}
                ...
                onClick={() => {
                  !checkSameBatch() && setEditDateVisible(true);
                }} key="edit">编辑填报截止日期</Button>
        <Button className={styles.btn}
        ...
        >取消发布</Button>
        <Button style={{ margin: '0 6px 0 3px' }} onClick={() => {
          handleSave();
        }}>保存</Button>
        <DataImport
