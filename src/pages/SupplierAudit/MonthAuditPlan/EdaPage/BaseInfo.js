 ...
 * @LastEditTime : 2020-12-21 13:41:02
 * @Description:  基本信息
      ...
      <div className={styles.bgw}>
        ...
        <div className={styles.content}>
          {type !== 'add' && (
            <Row>
              <Col span={12}>
                <FormItem label="月度审核计划号" {...formLayout} style={{ marginBottom: '0px' }}>
                  {isView ? (
                    <span>{data.reviewPlanMonthCode}</span>
                  ) : (
          ...
          <Row>
            <Col span={12}>
              <FormItem label="月度" {...formLayout} style={{ marginBottom: '0px' }}>
                {isView ? (
                  <span>{data.applyMonth ? data.applyMonth.slice(0, 7) : ''}</span>
                ) : (
                  getFieldDecorator('applyMonth', {
                    initialValue: type === 'add' ? '' : data.applyMonth && moment(data.applyMonth),
                    rules: [
                      {
                        required: true,
                        message: '月度不能为空',
          ...
          <Row>
            <Col span={24}>
              <FormItem label="拟制说明" {...formLongLayout} style={{ marginBottom: '0px' }}>
                {isView ? (
                  <span>{data.reviewPlanMonthName}</span>
                ) : (
