...
const AddModal = (props) => {
  ...
  const columns = [
    {
      title: '月度审核计划号和行号',
      dataIndex: 'reviewPlanMonthCode',
      width: 200,
      ellipsis: true,
      render: (text, item) => text + ' ' + item.reviewPlanMonthLinenum,
    },
    {
      title: '审核月度',
      ...
      width: 100,
      ellipsis: true,
      render: () => applyMonth ? applyMonth.slice(0, 7) : 0,
    },
    {
  ...
  const onOk = async () => {
    ...
    handleCancel();
    openNewTab(`supplierAudit/AuditImplementationPlan/editPage?pageState=add&ids=${selectedRowKeys.join()}&reviewPlanMonthId=${getFieldValue('reviewPlanMonthId')}`, '新增审核实施计划', false);
  };
  ...
  >
    <div>
      <Row>
          ...
          <FormItem {...formItemLayoutLong} label={'月度审核计划'}>
            {
              ...
              getFieldDecorator('reviewPlanMonthId'),
              ...
              field={['reviewPlanMonthCode', 'reviewPlanMonthId']}
              afterSelect={(item) => {
              setCascadeParams({
        ...
        <Col span={4}>
          <div style={{ textAlign: 'center', marginTop: '3px' }} onClick={handleSearch}><Button
            type="primary">查询</Button></div>
        </Col>
      </Row>
    </div>
    <ExtTable
