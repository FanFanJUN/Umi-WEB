  ...
  return <Spin spinning={loading}>
    <Affix>
      <div className={classnames(styles.fbc, styles.affixHeader)}>
        ...
        {
          props.isInFlow !== 1 && (data.type !== 'detail' || data.type === 'change' || data.type === 'edit') &&
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <Button className={styles.btn} onClick={() => {
              closeCurrent();
            }}>返回</Button>
            {data.type !== 'change' &&
            <Button className={styles.btn} onClick={() => handleSave('save')} loading={loading}>暂存</Button>}
            {data.type === 'change' &&
            <Button className={styles.btn} onClick={() => handleSave('delete')} loading={loading}>作废</Button>}
            <StartFlow
