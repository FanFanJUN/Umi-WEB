...
const Index = () => {
  ...
  const buttonClick = async (type) => {
    switch (type) {
      case 'add':
        setData((value) => ({ ...value, visible: true, title: '审核准则管理新增', type: 'add' }));
        break;
      case 'edit':
        setData((value) => ({ ...value, visible: true, title: '审核准则管理编辑', type: 'edit' }));
        break;
      case 'delete':
        await deleteData();
        break;
  ...
  const headerLeft = <div style={{ width: '100%', display: 'flex', height: '100%', alignItems: 'center' }}>
    {
      authAction(<Button
        ...
        key='SUPPLIER_AUDIT_CRITERIA_ADD'
      >新增</Button>)
    }
    {
      authAction(<Button
        ...
        key='SUPPLIER_AUDIT_CRITERIA_EDIT'
      >编辑</Button>)
    }
    {/*{*/}
    {/*  authAction(<Button*/}
    ...
    {/*    key='SUPPLIER_AUDIT_CRITERIA_DELETE'*/}
    {/*  >删除</Button>)*/}
    {/*}*/}
    {
      authAction(<Button
        ...
        className={styles.btn}
        ignore={DEVELOPER_ENV}
        key='SUPPLIER_AUDIT_CRITERIA_FORST'
