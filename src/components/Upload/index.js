          ...
          if (res.success && res.data && res.data.length > 0) {
            res.data.map(item => {
              fileList.push({
                ...
                thumbUrl: host + '/api-gateway/edm-service/preview?docId=' + item.id,
  ...
  render() {
    ...
    return (
      <div className={'upload-style'} style={this.props.style}>
        ...
        <span style={this.state.completeUploadFile.length !== 0 && this.props.showColor ? { cursor: 'pointer', ...this.props.showStyle } : { cursor: 'pointer' }} onClick={() => this.handleModalVisible(true)}>
          {children || ''}
